...
public class SortRecursiveAction extends RecursiveAction {
    ...
    public SortRecursiveAction(int[] array, int start, int end) {
        this.array = array;
        this.start = start;
        this.end = end;
    }
    ...
    protected void compute() {
        ...
        int mid = (end - start) / 2;
        SortRecursiveAction left = new SortRecursiveAction(array, start, start + mid);
        SortRecursiveAction right = new SortRecursiveAction(array, start + mid + 1, end);
        // 执行子任务
        left.fork();
        right.fork();
        //等待子任务都执行完成
