...
class JvmInfoUtils {
    ...
    public static MachineInfo.JvmInfo printJvmInfo() {
        ...
        jvmInfo.setTotal(UnitConversionUtils.formatByte(jvmTotalMemoryByte));
        jvmInfo.setMax(UnitConversionUtils.formatByte(jvmMaxMoryByte));
        jvmInfo.setFree(UnitConversionUtils.formatByte(freeMemoryByte));
    ...
    public static MachineInfo.GcInfo printGcUtilInfo() {
        ...
        try {
            ...
            Process process = Runtime.getRuntime().exec(javaHome + "/bin/jstat -gcutil " + ProcessUtils.PID);
