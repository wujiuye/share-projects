...
public final class Jyson {
	 ...
	 * @return 返回指定对象的类型
	 */
	public <T> Object parseJson(String json, Class<T> toClass) {
		try {
			//获取类上的注解值
			...
			if (jcName != null) {
				...
				if (jcType == 0) {
					JSONObject findResult = null;
					// 根据类对象的注解在json获取表示该对象的字符串
					// 使用递归（深度遍历）
					//findResult = findClassAtJsonField(jsonObject, jcName);
					// 使用队列（广度遍历）
					findResult = findClassAtJsonField_Queue(jsonObject, jcName);
					if (findResult != null) {
			...
			for (int i = 0; i < fs.length; i++) {
				...
				try {
					if (type.endsWith("String")) {
                        ...
                        parseJson(json.getJSONObject(jsonFiledName),
                                sobj);
                    }
